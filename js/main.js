...
const ppriceTotal = document.querySelector('.Total');
...
async function agregarAlCarrito(e) {
    ...
    let prodElegido = productos.filter(prd => prd.id == e.target.getAttribute('prodID'));
    let titleProdElegido = prodElegido[0].title;
    mostrarToast(titleProdElegido);
...
function mostrarToast(prod) {
    // Muestra una notificacion en forma de toast con el title del producto agregado al carrito
    Toastify({
...
async function mostrarCarrito() {
    ...
    carritoSinDuplicados.forEach((item) => {
        ...
        div.innerHTML = `
                        <img src=${miItem[0].image} alt=${miItem[0].title} class="imgProdCarrito">
                        <p>${miItem[0].title}</p>
                        <p>Cantidad: ${unidadesProd}</p>
                        <p>price: $${miItem[0].price}</p>
                        `
...
function actualizarContadorCarrito() {
    // Muestra un contador de los items del carrito a la derecha de la image del carrito
...
function verificarEstadoCarrito() {
    ...
    carrito.length == 0 ?
        (
            ppriceTotal.setAttribute('style', 'display:none'),
            ...
            ppriceTotal.removeAttribute('style', 'display:none'),
    ...
    if (seleccionados.length == 0) {
        ...
        for (let i = 0; i < seleccionados.length; i++) {
            for (let p = 0; p < productos.length; p++) {
                seleccionados[i] == productos[p].category && prodCheckeados.push(productos[p])
            }
        }
